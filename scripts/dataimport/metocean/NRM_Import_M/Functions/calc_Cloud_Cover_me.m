...
Daily_Cloud(1:length(mDate),1) = 0;
...
for ii = 1:length(mday)
    ...
    if ii == length(mday) | length(mday) - ii == 1% this option  is intended to fill the gap in the evening of last day from sunset to 23:00
    ...
    if    length(mday) > ii  & (length(mday) - ii) > 1 %  ii = 1 is incluled ,because i need to fill the gap at night of first day.
    
%        disp([num2str(length(mday)),' ',num2str(ii)]);
