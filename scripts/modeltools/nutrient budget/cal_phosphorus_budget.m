...
for ss=1:length(sites)
 ...
 %   datess={'20190101','20190401','20190701','20191001','20200101'};
    outputfolder=['./Budget_GEN2_4yrs_TP_Dcorr/',site,'/'];
    ...
    define_phosphorus_outputs_v6;
    
    readdata=0;
    
    if readdata
        data=[];
        preprocessing_vars_phosphorus;
...
%         data = cal_2D_pool(data,infolder, NPool_2D,t1,t2,NPool_2D_factors);
    ...
    data.PHYTO_BURIAL_P=data.WQ_DIAG_PHY_PHY_SWI_P-data.MPB_RESXPC;
   % data.PHYTO_BURIAL=-data.WQ_DIAG_PHY_MPB_BEN*10/106*0.2;
    ...
    datesv=datenum(datess,'yyyymmdd');
    set(gca,'XTick',datesv,'XTickLabel',datestr(datesv,'mm-yy'));
    title('(b) phosphorus sources and sinks (+ve: into water; -ve: into sediment)','FontWeight','Bold','FontSize',fs);
    ...
    data.nsnetflux.IP=data.nsnetflux.PHS_frp+data.nsnetflux.PHS_frp_ads;
    data.nsnetflux.OP=data.nsnetflux.OGM_dop+data.nsnetflux.OGM_pop;
    data.nsnetflux.PP=data.nsnetflux.MA2_ulva_IP+data.nsnetflux.PHY_grn*0.002976...
        +data.nsnetflux.PHY_crypt*0.002809+data.nsnetflux.PHY_diatom*0.00186+data.nsnetflux.PHY_dino*0.003106;
    ...
    for ii=[2 3 4 5 7] %2:length(N_BGC_vars)
        if ii==2
    ...
    Ttran=data.nsnetflux.IP+data.nsnetflux.OP+data.nsnetflux.PP;
    ...
    hold on;
    plot(t1:t2,-Sedacc*31/1e9,'Color',[136,86,167]./255);
    hold on;
%     plot(t1:t2,-Sedacc2*31/1e9,'Color','k');
%     hold on;
    
        str1={['total: ',num2str(Watacc(end)*31/1e9,'%4.2f'),' Tonnes']};
    text(t2+5,Watacc(end)*31/1e9,str1, 'Color',[67,162,202]./255);
    hold on;
       str1={['total: ',num2str(-Sedacc(end)*31/1e9,'%4.2f'),' Tonnes']};
    text(t2+5,-Sedacc(end)*31/1e9,str1, 'Color',[136,86,167]./255);
    
    set(gca,'xlim',[t1 t2]); %,'ylim',[-15 5]);
    
    datesv=datenum(datess,'yyyymmdd');
    set(gca,'XTick',datesv,'XTickLabel',datestr(datesv,'mm-yy'));
    xlabel('');ylabel({'cumulative change','(tonnes)'});
    %set(gca,'FontSize',9);
    title('(d) cumulative change in phosphorus pools','FontWeight','Bold','FontSize',fs);
    box on;grid on;
    
    hl=legend('phosphorus in water','phosphorus in sediment');
    set(hl,'Fontsize',fsl,'Position',pos41);
    
    outputName=[outputfolder,sitenames{ss},'_phosphorus_budget_timeseries.png'];
    ...
    title('(b) phosphorus fluxes','FontWeight','Bold','FontSize',fs);
    xlabel('');ylabel({'phosphorus fluxes','(tonnes/day)'});
    ...
    hh1 = bar(1:length(cc3),cc3'*31/1e9,0.6,'stacked');hold on;
    ...
    xlabel('');ylabel({'Boundary fluxes','(tonnes/day)'});
    %set(gca,'FontSize',9);
    title('(d) Boundary phosphorus fluxes','FontWeight','Bold','FontSize',fs);
    box on;grid on;
    
    outputName=[outputfolder,'phosphorus_budget_overall.png'];
    print(gcf,'-dpng',outputName);
    ...
    outputName=[outputfolder,'phosphorus_budget_overall_sum.png'];
    print(gcf,'-dpng',outputName);
