...
fprintf(fid,'Variable,River,Coorong,Lakes/n');
...
for i = 1:length(init.(vars{4}))
    ...
    for j = 1:length(vars)
        if j == length(vars)
            fprintf(fid,'%5.4f/n',init.(vars{j})(i));
        else
