...
input_file = '014_Coorong_Salt_Crk_Mouth_Channel_MZ3_Culverts.2dm';
output_file = '015_Coorong_MAT.2dm';
...
[XX,YY,nodeID,faces,X,Y,ID] = tfv_get_node_from_2dm(input_file);

fid_in = fopen(input_file,'rt');
fid_out = fopen(output_file,'wt');

% Copy out the header lines
fline = fgetl(fid_in);
fprintf(fid_out,'%s\n',fline);
fline = fgetl(fid_in);
fprintf(fid_out,'%s\n',fline);
...
for i = 1:length(X)
    ...
    original_val = str2num(str{end});
    %original_val = outside_zone;
...
%     if original_val == outside_zone
%         
%         dtri = DelaunayTri(polX',polY');
%         pt_id = nearestNeighbor(dtri,[X(i) Y(i)]);
%         
%         original_val = shp(pt_id).id;
%         
%         
%     end
    ...
    for k = 1:length(str)-1
        ...
        fprintf(fid_out,'%s ',str{k});
        
    end
    
    fprintf(fid_out,'%s\n',num2str(original_val));
    
    clear mat_val original_val;
end
