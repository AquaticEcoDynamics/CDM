...
load ../../../../data/store/hydro/dew_WaterDataSA_hourly_STP.mat
...
[ttt.Date,ind1] = unique(dew.A4261039.SAL.Date);
ttt.Data = dew.A4261039.SAL.Data(ind1);

data.SAL = [];
data.SAL = interp1(ttt.Date,ttt.Data,data.Date);
...
fid = fopen('VH_20120101_20210701_v7_BK_SAL.csv','wt');
for i = 1:length(vars)
    if i == length(vars)
        fprintf(fid,'%s\n',vars{i});
    else
