function create_tfv_inflow_file_v2(data,headers,datearray,filename,X,Y,subdir,sitename,zero_missing)
...
for i = 1:length(headers)
    ...
    if site_ID < 900
        if strcmpi(headers{i},'H') == 0
            ...
            if zero_missing
            tfv_data.(headers{i}) = create_missing_dataset...
...
function var = create_missing_dataset(data,varname,depth,mtime)
...
t_date_NoNaN_years = mtime;
t_data_NoNaN_years(1:length(mtime),1) = 0;
for kk = 1:length(mtime)
    sss = find(floor(t_date_NoNaN) == floor(mtime(kk)));
    if ~isempty(sss)
        t_data_NoNaN_years(kk) = t_data_NoNaN(sss);
    end
end
