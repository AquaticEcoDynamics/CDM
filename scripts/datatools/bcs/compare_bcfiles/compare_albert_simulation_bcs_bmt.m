...
outdir = 'Images_BMT/COMP/'; mkdir(outdir);

albert_old = tfv_readBCfile(['BMT\',...
    'LAC_BC_20140101_20200701_800GLy-1_realistic.csv']);

albert_old_ext = tfv_readBCfile(['BMT\',...
    'LAC_BC_20130101_20200701_800GLy-1_realistic_ext.csv']);

albert_new = tfv_readBCfile(['Phase2\',...
    'Albert_Phase_II_20130101_20200701_AED.csv']);

% river_new = tfv_readBCfile(['Phase2\',...
%     'River_Phase_II_20140101_20200701.csv']);

vars = fieldnames(albert_new);
...
for i = 1:length(vars)
    ...
    if  strcmpi(vars{i},'Date') == 0 
        ...
        plot(albert_old_ext.Date,albert_old_ext.(vars{i}),'--');hold on
...
%         plot(river_new.Date,river_new.(vars{i}));hold on
        ...
        legend({'Albert ph.1';'Albert ph.1 ext';'Albert ph.2'});
 ...
 create_html_for_directory('Images_BMT/');
