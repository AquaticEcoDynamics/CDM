...
Each version of TUFLOW-FV models in the CIIP and HCHB project folders are organised with the following structure:
...
The processing of any model output is predominately written and run in Matlab. Scripts that have been specifically developed for this project are contained within the ```scripts/modeltools``` directory. Scripts that have created figures 3.9, 3.11 and 5.2 (among others) can be found here. Examples plotting and processing types include:

  + Ruppia HSI Processing & DelMap Plotting;
...
Plottfv_transect can also be found in the aed_matlab_modeltools repository. Unlike plottfv_polygon, plottfv_transect plots model data extracted along a transect line during a specified plotting period, and compares against the range of field data found within that period.
